import { a as TypedEventTarget, i as createFrameHandle, n as Frame, o as jsx, r as createComponent, t as Fragment } from "./component-CBgljxmI.js";
//#region node_modules/@remix-run/component/dist/lib/error-event.js
/**
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/invariant.js
/**
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/document-state.js
/**
...
function createDocumentState(_doc) {
	...
	function setSelection(input, offsets) {
		if ("selectionStart" in input && "selectionEnd" in input) try {
			let htmlInput = input;
	...
	function restoreSelection(priorSelectionInformation) {
		...
		if (curFocusedElem !== priorFocusedElem && priorFocusedElem && isInDocument(priorFocusedElem)) {
			...
			if (priorFocusedElem instanceof HTMLElement && typeof priorFocusedElem.focus === "function") priorFocusedElem.focus();
	...
	function restore() {
		if (selectionInfo !== null) restoreSelection(selectionInfo);
		selectionInfo = null;
	}
	return {
		capture,
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/vnode.js
...
function isCommittedTextNode(node) {
	return isTextNode$1(node) && node._dom instanceof Text;
}
...
function isRemixElement$1(node) {
	return typeof node === "object" && node !== null && "$rmx" in node;
}
function findContextFromAncestry(node, type) {
	let current = node;
	while (current) {
		if (current.type === type && isComponentNode(current)) return current._handle.getContextValue();
		current = current._parent;
	}
}
//#endregion
//#region node_modules/@remix-run/component/dist/lib/style/lib/stylesheet.js
...
function walkRulesForSelectors(rules, layerName, addSelector) {
	for (let i = 0; i < rules.length; i++) {
		...
		if (childRules) walkRulesForSelectors(childRules, nextLayerName, addSelector);
	}
}
function seedManagersWithServerSelectors(layerName, selectors) {
	for (let mgr of activeManagers) {
		if (mgr.layer !== layerName) continue;
		for (let selector of selectors) if (!mgr.ruleMap.has(selector)) mgr.ruleMap.set(selector, {
			count: 1,
			index: -1
		});
	}
...
function startServerStyleObserver() {
	...
	serverStyleState.observer.observe(root, {
		childList: true,
		subtree: true
	});
}
function adoptServerStyleTag(styleEl) {
	...
	try {
		if (styleEl.sheet) walkRulesForSelectors(styleEl.sheet.cssRules, null, addSelector);
	} catch {}
	let adopted = false;
	let cssText = styleEl.textContent?.trim() ?? "";
	if (cssText.length === 0) adopted = true;
	else if (serverStyleState.adoptedTexts.has(cssText)) adopted = true;
	else try {
	...
	if (adopted) styleEl.remove();
	for (let [layerName, selectors] of addedSelectorsByLayer) seedManagersWithServerSelectors(layerName, selectors);
}
function teardownServerStyleStateIfUnused() {
	if (!serverStyleState) return;
	if (serverStyleState.refCount > 0) return;
	if (serverStyleState.observer) serverStyleState.observer.disconnect();
...
function createStyleManager(layer = "rmx") {
	...
	if (serverSelectors) for (let selector of serverSelectors) ruleMap.set(selector, {
		count: 1,
		index: -1
	});
	let manager = {
	...
	function remove(className) {
		...
		for (let [, data] of ruleMap.entries()) if (data.index > indexToDelete) data.index--;
	}
	function dispose() {
		if (stylesheet) document.adoptedStyleSheets = Array.from(document.adoptedStyleSheets).filter((s) => s !== stylesheet);
		ruleMap.clear();
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/style/lib/style.js
...
function normalizeCssValue(key, value) {
	...
	if (typeof value === "number" && value !== 0) {
		let cssKey = camelToKebab$1(key);
		if (!NUMERIC_CSS_PROPS.has(cssKey) && !cssKey.startsWith("--")) return `${value}px`;
	}
	return String(value);
}
...
function styleToCss(styles, selector = "") {
	...
	let atRules = [];
	let preludeAtRules = [];
	for (let [key, value] of Object.entries(styles)) if (isComplexSelector(key)) {
		if (key.startsWith("@")) {
			let record = toRecord(value);
			if (!record) continue;
			if (key.startsWith("@function")) {
				let body = atRuleBodyToCss(record);
				if (body.trim().length > 0) preludeAtRules.push(`${key} {\n${indent(body, 2)}\n}`);
				else preludeAtRules.push(`${key} {\n}`);
				...
				if (inner.trim().length > 0) atRules.push(`${key} {\n${indent(inner, 2)}\n}`);
				else atRules.push(`${key} {\n  ${selector} {\n  }\n}`);
			}
			continue;
		}
		let record = toRecord(value);
		if (!record) continue;
		let nestedContent = "";
		for (let [prop, propValue] of Object.entries(record)) if (propValue != null) {
			let normalizedValue = normalizeCssValue(prop, propValue);
			nestedContent += `    ${camelToKebab$1(prop)}: ${normalizedValue};\n`;
		}
		if (nestedContent) nestedBlocks.push(`  ${key} {\n${nestedContent}  }`);
	} else if (value != null) {
		let normalizedValue = normalizeCssValue(key, value);
		baseDeclarations.push(`  ${camelToKebab$1(key)}: ${normalizedValue};`);
	}
	let css = "";
	if (preludeAtRules.length > 0) css += preludeAtRules.join("\n");
	if (selector && (baseDeclarations.length > 0 || nestedBlocks.length > 0)) {
		css += (css ? "\n" : "") + `${selector} {\n`;
		if (baseDeclarations.length > 0) css += baseDeclarations.join("\n") + "\n";
		if (nestedBlocks.length > 0) css += nestedBlocks.join("\n") + "\n";
		css += "}";
	}
	if (atRules.length > 0) css += (css ? "\n" : "") + atRules.join("\n");
	return css;
}
...
function keyframesBodyToCss(frames) {
	...
	for (let [frameSelector, frameValue] of Object.entries(frames)) {
		if (!isRecord(frameValue)) continue;
		let declarations = [];
		for (let [prop, propValue] of Object.entries(frameValue)) {
			if (propValue == null) continue;
			if (isComplexSelector(prop)) continue;
			let normalizedValue = normalizeCssValue(prop, propValue);
			declarations.push(`  ${camelToKebab$1(prop)}: ${normalizedValue};`);
		}
		if (declarations.length > 0) blocks.push(`${frameSelector} {\n${declarations.join("\n")}\n}`);
		else blocks.push(`${frameSelector} {\n}`);
	}
	return blocks.join("\n");
}
function atRuleBodyToCss(styles) {
	let declarations = [];
	let nested = [];
	for (let [key, value] of Object.entries(styles)) if (isComplexSelector(key)) if (key.startsWith("@")) {
		let record = toRecord(value);
		if (!record) continue;
		let inner = atRuleBodyToCss(record);
		if (inner.trim().length > 0) nested.push(`${key} {\n${indent(inner, 2)}\n}`);
		else nested.push(`${key} {\n}`);
	} else continue;
	else if (value != null) {
		let normalizedValue = normalizeCssValue(key, value);
		declarations.push(`  ${camelToKebab$1(key)}: ${normalizedValue};`);
	}
	let body = "";
	if (declarations.length > 0) body += declarations.join("\n");
	if (nested.length > 0) body += (body ? "\n" : "") + nested.join("\n");
	return body;
}
function processStyleClass(styleObj, styleCache) {
	...
	let result = {
		selector,
		css: styleToCss(styleObj, `.${selector}`)
	};
	styleCache.set(hash, result);
	return result;
}
//#endregion
//#region node_modules/@remix-run/component/dist/lib/svg-attributes.js
var XLINK_NS = "http://www.w3.org/1999/xlink";
var XML_NS = "http://www.w3.org/XML/1998/namespace";
var CANONICAL_CAMEL_SVG_ATTRS = new Set([
	"accentHeight",
	"attributeName",
	"attributeType",
	"baseFrequency",
...
for (let attr of CANONICAL_CAMEL_SVG_ATTRS) SVG_ATTR_ALIASES.set(camelToKebab(attr), attr);
var NAMESPACED_SVG_ALIASES = new Map([
	["xlinkHref", {
		ns: XLINK_NS,
		attr: "xlink:href"
	}],
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/diff-props.js
var SVG_NS$1 = "http://www.w3.org/2000/svg";
var defaultStyleManager = typeof window !== "undefined" ? createStyleManager() : null;
...
function clearRuntimePropertyOnRemoval(dom, name) {
	try {
		...
		if (name === "selectedIndex") dom[name] = -1;
	} catch {}
}
function getMergedClassName(props) {
	...
	return (classAttr && className ? `${classAttr} ${className}` : classAttr || className) || void 0;
}
function diffHostProps(curr, next, dom) {
	let isSvg = dom.namespaceURI === SVG_NS$1;
	let currClassName = getMergedClassName(curr);
	let nextClassName = getMergedClassName(next);
	if (currClassName !== nextClassName) if (nextClassName) dom.setAttribute("class", nextClassName);
	...
	for (let name in curr) {
		...
		if (!(name in next) || next[name] == null) {
			if (canUseProperty(dom, name, isSvg)) clearRuntimePropertyOnRemoval(dom, name);
	...
	for (let name in next) {
		...
		if (curr[name] !== nextValue) {
			...
			if (canUseProperty(dom, name, isSvg)) try {
			...
			} else if (ns) dom.removeAttributeNS(ns, toLocalName(attr));
			else dom.removeAttribute(attr);
		}
	}
}
//#endregion
//#region node_modules/@remix-run/component/dist/lib/client-entries.js
function clientEntry(href, component) {
	let [moduleUrl, exportName] = href.split("#");
	if (!moduleUrl) throw new Error("clientEntry() requires a module URL");
	let finalExportName = exportName || component.name;
	if (!finalExportName) throw new Error("clientEntry() requires either an export name in the href (e.g., \"/js/module.js#ComponentName\") or a named component function");
...
function skipComments(cursor) {
	while (cursor && cursor.nodeType === Node.COMMENT_NODE) cursor = cursor.nextSibling;
	return cursor;
}
//#endregion
//#region node_modules/@remix-run/component/dist/lib/to-vnode.js
...
function flattenRemixNodeArray(nodes, out = []) {
	for (let child of nodes) if (Array.isArray(child)) flattenRemixNodeArray(child, out);
	else out.push(child);
	return out;
}
function toVNode(node) {
	...
	if (Array.isArray(node)) return {
		type: Fragment,
		_children: flattenRemixNodeArray(node).map(toVNode)
	};
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/mixin.js
...
function resolveMixedProps(input) {
	...
	for (let index = 0; index < descriptors.length && index < maxDescriptors; index++) {
		...
		if (!entry || entry.type !== descriptor.type) {
			if (entry) queueMixinRemove(handle, entry.scope);
			...
			if (binding?.node) queueMixinInsert(handle, entry.scope, binding.node, binding.parent, binding.key);
		}
		...
		if (!isRemixElement(result)) {
			console.error(/* @__PURE__ */ new Error("mixins must return a remix element"));
			continue;
		}
		...
		if (resultType !== hostType) {
			console.error(/* @__PURE__ */ new Error("mixins must return an element with the same host type"));
			continue;
		}
	...
	if (state.runners.length > descriptors.length) state.runners.length = descriptors.length;
...
function bindMixinRuntime(state, binding, options) {
	...
	let handle = state.handle;
	if (!handle) return;
	for (let entry of state.runners) if (options?.dispatchReclaimed) queueMixinReclaimed(handle, entry.scope, nextNode, nextBinding.parent, nextBinding.key);
	else queueMixinInsert(handle, entry.scope, nextNode, nextBinding.parent, nextBinding.key);
}
function prepareMixinRemoval(state) {
	...
	let handle = state.handle;
	if (!handle) return;
	for (let entry of state.runners) dispatchMixinBeforeRemove(handle, entry.scope, registerPersistNode);
...
var MixinHandleImpl = class extends TypedEventTarget {
	...
	addEventListener(type, listener, options) {
		this.#getActiveScopeTarget().addEventListener(type, listener, options);
		...
		if (type === "beforeUpdate") this.#options.scheduler.addEventListener("beforeUpdate", this.#onSchedulerBeforeUpdate);
		else this.#options.scheduler.addEventListener("commit", this.#onSchedulerCommit);
	}
	removeEventListener(type, listener, options) {
		this.#getActiveScopeTarget().removeEventListener(type, listener, typeof options === "boolean" ? { capture: options } : options);
		if (!listener || !isSchedulerPhaseType(type)) return;
		let scope = this.#activeScope;
		invariant(scope);
		let scopePhaseCounts = this.#scopePhaseCounts.get(scope);
		...
		if (type === "beforeUpdate") this.#options.scheduler.removeEventListener("beforeUpdate", this.#onSchedulerBeforeUpdate);
		else this.#options.scheduler.removeEventListener("commit", this.#onSchedulerCommit);
	}
	update() {
	...
	releaseScope(scope) {
		...
		if (this.#activeScope === scope) this.#activeScope = void 0;
	}
...
function dispatchMixinRemoveEvent(state) {
	...
	let handle = state?.handle;
	if (!handle) return;
	for (let entry of runners) handle.dispatchScopedEvent(entry.scope, new Event("remove"));
}
function finalizeMixinTeardown(state) {
	dispatchMixinRemoveEvent(state);
	let handle = state.handle;
	if (handle) for (let entry of state.runners) handle.releaseScope(entry.scope);
...
function normalizeMixinRunner(result, handle) {
	if (typeof result === "function" && !isMixinElement(result)) return result;
	if (result === void 0) return () => handle.element;
	return () => result;
}
//#endregion
//#region node_modules/@remix-run/component/dist/lib/reconcile.js
...
function restoreControlledReflections(node, state) {
	...
	if (state.hasControlledValue && readDomProp(element, "value") !== state.controlledValue) setPropertyReflection(element, "value", state.controlledValue);
	if (state.hasControlledChecked && readDomProp(element, "checked") !== state.controlledChecked) setPropertyReflection(element, "checked", state.controlledChecked);
}
...
function enqueueMixinBindingUpdate(done) {
	...
	this.scheduler.enqueueWork([() => {
		...
		diffHostProps(getHostProps(node), resolveNodeMixProps(node, this.frame, this.scheduler, state), this.node);
...
function getDocumentHead(domParent) {
	if (domParent instanceof Document) return domParent.head;
	if (domParent instanceof Node) return domParent.ownerDocument?.head ?? null;
	return null;
}
function diffVNodes(curr, next, domParent, frame, scheduler, styles, vParent, rootTarget, anchor, rootCursor) {
	...
	if (curr === null) return insert(next, domParent, frame, scheduler, styles, vParent, rootTarget, anchor, rootCursor);
	if (curr.type !== next.type) {
		replace(curr, next, domParent, frame, scheduler, styles, vParent, rootTarget, anchor);
		return rootCursor;
	}
	if (isCommittedTextNode(curr) && isTextNode$1(next)) {
	...
	if (isFragmentNode(curr) && isFragmentNode(next)) {
		diffChildren(curr._children, next._children, domParent, frame, scheduler, styles, vParent, rootTarget, void 0, anchor);
		return rootCursor;
	}
...
function diffHost(curr, next, frame, scheduler, styles, vParent, rootTarget) {
	...
	if (shouldDispatchInlineMixinLifecycle(curr._dom)) dispatchMixinBeforeUpdate(next._mixState);
	if (nextProps.innerHTML != null) {
		if (currProps.innerHTML !== nextProps.innerHTML) curr._dom.innerHTML = nextProps.innerHTML;
	} else if (currProps.innerHTML != null) curr._dom.innerHTML = "";
	...
	if (shouldDispatchInlineMixinLifecycle(curr._dom)) scheduler.enqueueCommitPhase([() => dispatchMixinCommit(next._mixState)]);
}
...
function diffText(curr, next, vParent) {
	if (curr._text !== next._text) curr._dom.textContent = next._text;
	next._dom = curr._dom;
	next._parent = vParent;
}
function insert(node, domParent, frame, scheduler, styles, vParent, rootTarget, anchor, cursor) {
	...
	if (cursor && anchor && cursor === anchor) cursor = null;
	...
	if (isHostNode(node)) {
			...
			if (targetHead) {
				...
				if (cursor instanceof Element && cursor.tagName.toLowerCase() === "head") {
					...
					if (cursor !== targetHead) {
						while (cursor.firstChild) targetHead.appendChild(cursor.firstChild);
						cursor.remove();
					}
					cursor = nextCursor;
				}
		...
		if (cursor instanceof Element) {
			...
			if (cursorTag === node.type) {
				...
				if (hostProps.innerHTML != null) cursor.innerHTML = hostProps.innerHTML;
				else {
				...
				if (nextSibling instanceof Element) {
					if ((node._svg ? nextSibling.tagName : nextSibling.tagName.toLowerCase()) === node.type) {
						let nextCursor = nextSibling.nextSibling;
						diffHostProps({}, hostProps, nextSibling);
						if (hostProps.innerHTML != null) nextSibling.innerHTML = hostProps.innerHTML;
						else {
		...
		if (hostProps.innerHTML != null) dom.innerHTML = hostProps.innerHTML;
		else diffChildren(null, node._children, dom, frame, scheduler, styles, node, rootTarget);
	...
	if (isFragmentNode(node)) {
		for (let child of node._children) cursor = insert(child, domParent, frame, scheduler, styles, vParent, rootTarget, anchor, cursor);
		return cursor;
	}
	if (isComponentNode(node)) return diffComponent(null, node, frame, scheduler, styles, domParent, vParent, rootTarget, anchor, cursor);
	if (node.type === Frame) return insertFrame(node, domParent, frame, scheduler, styles, vParent, rootTarget, anchor, cursor);
...
function diffFrame(curr, next, domParent, frame, scheduler, styles, vParent, rootTarget, anchor) {
	...
	if (getFrameName(curr) !== getFrameName(next)) {
	...
	if (currSrc !== nextSrc) {
		...
		if (frameInstance) frameInstance.handle.src = nextSrc;
		let runtime = getFrameRuntime(frame);
		if (runtime) resolveClientFrame(next, runtime);
	}
	if (!next._frameResolved && next._frameFallbackRoot) next._frameFallbackRoot.render(next.props?.fallback ?? null);
}
function insertFrame(node, domParent, frame, scheduler, styles, vParent, rootTarget, anchor, cursor) {
	let runtime = getFrameRuntime(frame);
	if (!runtime || runtime.canResolveFrames === false) throw new Error("Cannot render <Frame /> without frame runtime. Use run() or pass frameInit to createRoot/createRangeRoot.");
...
function resolveClientFrame(node, runtime) {
	...
	Promise.resolve(runtime.resolveFrame(frameSrc, resolveController.signal)).then(async (content) => {
		...
		if (node._frameResolveController === resolveController) node._frameResolveController = void 0;
	});
}
...
function removeFrameDomRange(node, domParent) {
	...
	while (cursor) {
		...
		if (cursor.parentNode === domParent) domParent.removeChild(cursor);
		if (cursor === end) break;
		cursor = nextSibling;
	}
...
function diffComponent(curr, next, frame, scheduler, styles, domParent, vParent, rootTarget, anchor, cursor) {
	if (curr === null) {
		...
		if (componentId) vParent._pendingHydrationComponentId = void 0;
		else componentId = `c${++idCounter}`;
		next._handle = createComponent({
			...
			getFrameByName(name) {
				return getFrameRuntime(frame)?.namedFrames.get(name);
			},
			getTopFrame() {
				return getFrameRuntime(frame)?.topFrame;
			}
		});
...
function cleanupDescendants(node, scheduler, styles) {
	if (isCommittedTextNode(node)) return;
	if (isCommittedHostNode(node)) {
		for (let child of node._children) cleanupDescendants(child, scheduler, styles);
...
function remove(node, domParent, scheduler, styles) {
	...
	if (isCommittedHostNode(node)) {
		...
		if (persistedRemoval) {
			...
			persistedRemoval.catch(() => {}).finally(() => {
				if (!node._persistedByMixins) return;
	...
	if (isFragmentNode(node)) {
		for (let child of node._children) remove(child, domParent, scheduler, styles);
		return;
	}
	if (isCommittedComponentNode(node)) {
...
function performHostNodeRemoval(node, domParent, scheduler, styles) {
	if (isHeadHostNode(node)) for (let child of node._children) remove(child, node._dom, scheduler, styles);
	else for (let child of node._children) cleanupDescendants(child, scheduler, styles);
	teardownMixins(node._mixState);
	teardownControlledReflection(node);
	if (!isHeadHostNode(node)) node._dom.parentNode?.removeChild(node._dom);
	if (node._controller) node._controller.abort();
}
function diffChildren(curr, next, domParent, frame, scheduler, styles, vParent, rootTarget, cursor, anchor) {
	...
	for (let i = 0; i < nextLength; i++) {
		...
		if (node && node.key != null) {
			...
			if (seenKeys.has(node.key)) {
				if (!duplicateKeys) duplicateKeys = /* @__PURE__ */ new Set();
				duplicateKeys.add(node.key);
			} else seenKeys.add(node.key);
		}
	}
	...
	if (curr === null) {
		for (let node of next) cursor = insert(node, domParent, frame, scheduler, styles, vParent, rootTarget, anchor, cursor);
		vParent._children = next;
		return cursor;
	}
	let currLength = curr.length;
	if (!hasKeys) {
		for (let i = 0; i < nextLength; i++) diffVNodes(i < currLength ? curr[i] : null, next[i], domParent, frame, scheduler, styles, vParent, rootTarget, anchor, cursor);
		if (currLength > nextLength) for (let i = nextLength; i < currLength; i++) {
			let node = curr[i];
	...
	for (let i = 0; i < oldChildrenLength; i++) {
		...
		if (c) {
			c._flags = 0;
			if (c.key != null) oldKeyMap.set(c.key, i);
		}
	}
	let skew = 0;
	...
	for (let i = 0; i < nextLength; i++) {
		...
		if (key != null) {
			...
			if (mapIndex !== void 0) {
				...
				if (candidate && (candidateFlags & MATCHED) === 0 && candidate.type === type) matchingIndex = mapIndex;
			}
		} else {
			let searchVNode = oldChildren[skewedIndex];
			let searchFlags = searchVNode?._flags ?? 0;
			if (searchVNode != null && (searchFlags & MATCHED) === 0 && searchVNode.key == null && type === searchVNode.type) matchingIndex = skewedIndex;
		}
		...
		if (matchingIndex !== -1) {
			...
			if (matchedOldVNode) matchedOldVNode._flags = (matchedOldVNode._flags ?? 0) | MATCHED;
		}
		let oldDom = matchedOldVNode && findFirstDomAnchor(matchedOldVNode);
		if (!matchedOldVNode || !oldDom) {
			if (matchingIndex === -1) {
				if (nextLength > oldChildrenLength) skew--;
				else if (nextLength < oldChildrenLength) skew++;
			}
			childVNode._flags = (childVNode._flags ?? 0) | INSERT_VNODE;
		} else if (matchingIndex !== i + skew) if (matchingIndex === i + skew - 1) skew--;
		else if (matchingIndex === i + skew + 1) skew++;
		else {
	...
	if (remainingOldChildren) for (let i = 0; i < oldChildrenLength; i++) {
		let oldVNode = oldChildren[i];
		if (oldVNode && ((oldVNode._flags ?? 0) & MATCHED) === 0) remove(oldVNode, domParent, scheduler, styles);
	}
	...
	for (let i = 0; i < nextLength; i++) {
		...
		diffVNodes(idx >= 0 ? oldChildren[idx] : null, childVNode, domParent, frame, scheduler, styles, vParent, rootTarget, anchor, cursor);
		...
		if (shouldPlace && firstDom && lastDom && firstDom.parentNode === domParent) {
			let target;
			if (lastPlaced === null) if (vParent._rangeStart && vParent._rangeStart.parentNode === domParent) target = vParent._rangeStart.nextSibling;
			else target = domParent.firstChild;
			else target = lastPlaced.nextSibling;
			if (target === null && anchor) target = anchor;
			if (target && domRangeContainsNode(firstDom, lastDom, target)) {} else if (firstDom !== target) moveDomRange(domParent, firstDom, lastDom, target);
		}
...
function findFirstDomAnchor(node) {
	...
	if (isFragmentNode(node)) for (let child of node._children) {
...
function findLastDomAnchor(node) {
	...
	if (isFragmentNode(node)) for (let i = node._children.length - 1; i >= 0; i--) {
...
function reclaimPersistedMixinNode(persistedNode, newNode, frame, scheduler, styles, vParent, rootTarget) {
	...
	if (shouldDispatchInlineMixinLifecycle(persistedNode._dom)) dispatchMixinBeforeUpdate(newNode._mixState);
	...
	if (shouldDispatchInlineMixinLifecycle(persistedNode._dom)) scheduler.enqueueCommitPhase([() => dispatchMixinCommit(newNode._mixState)]);
}
//#endregion
//#region node_modules/@remix-run/component/dist/lib/scheduler.js
	...
	function flush() {
		...
		try {
			while (true) {
				...
				scheduled.clear();
				if (!(batch.size > 0 || workTasks.length > 0 || commitPhaseTasks.length > 0 || postCommitTasks.length > 0)) return;
				cascadingUpdateCount++;
				scheduleCounterReset();
				if (cascadingUpdateCount > MAX_CASCADING_UPDATES) {
					dispatchError(/* @__PURE__ */ new Error("handle.update() infinite loop detected"));
					return;
				}
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/vdom.js
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/diff-dom.js
function diffNodes(curr, next, context) {
	...
	for (let i = 0; i < max; i++) {
		let c = curr[i];
		let n = next[i];
		if (!c && n) if (regionTailRef) parent.insertBefore(n, regionTailRef);
			...
			if (cursor) i = next.indexOf(cursor);
		}
	}
}
function diffNode(current, next, context) {
	...
	if (isVirtualRootStartMarker(current) && isVirtualRootStartMarker(next)) {
		...
		if (current.data !== nextData) current.data = nextData;
		return findHydrationEndMarker(next);
	}
...
function diffElementAttributes(current, next) {
	...
	for (let name of prevAttrNames) if (!nextNameSet.has(name)) {
...
function shouldPreserveLiveAttribute(current, next, name) {
	if (name === "open") {
		if (current instanceof HTMLDetailsElement && next instanceof HTMLDetailsElement) return current.open !== next.open;
		if (current instanceof HTMLDialogElement && next instanceof HTMLDialogElement) return current.open !== next.open;
	}
	if (name === "checked") {
		if (current instanceof HTMLInputElement && next instanceof HTMLInputElement) return current.checked !== next.checked;
	}
	if (name === "value") {
		if (current instanceof HTMLInputElement && next instanceof HTMLInputElement && shouldPreserveInputValue(current)) return current.value !== next.value;
	}
	if (name === "selected") {
		if (current instanceof HTMLOptionElement && next instanceof HTMLOptionElement) return current.selected !== next.selected;
	}
	if (name === "popover") return isPopoverOpen(current) !== isPopoverOpen(next);
	return false;
}
function shouldPreserveElementChildren(current, next) {
	if (current instanceof HTMLTextAreaElement && next instanceof HTMLTextAreaElement) return current.value !== next.value;
	return false;
}
...
function diffElementChildren(current, next, context) {
	...
	for (let i = 0; i < nextChildren.length; i++) {
		...
		if (matchIndex === -1) {
			...
			if (candidateIndex < currentChildren.length && !used[candidateIndex] && nodeTypesComparable(currentChildren[candidateIndex], nextChild)) matchIndex = candidateIndex;
		}
		...
		if (mi !== -1) {
			...
			if (cursor) {
				...
				for (let j = i + 1; j <= nextEndIdx; j++) {
					...
					if (matchedIndex > currEndIdx) used[matchedIndex] = false;
			...
			committed[i] = curChild;
		} else committed[i] = nextChildren[i];
	}
	let anchor = void 0;
	for (let i = committed.length - 1; i >= 0; i--) {
		...
		if (isVirtualRootStartMarker(node) || isVirtualRootEndMarker(node)) {
			if (node.parentNode !== current) current.insertBefore(node, ref);
			anchor = node;
			continue;
		}
		if (node.parentNode === current) {
			let targetNext = ref;
			if (!(targetNext === null && node.nextSibling === null || node.nextSibling === targetNext)) current.insertBefore(node, targetNext);
		} else current.insertBefore(node, ref);
		if (node.parentNode === current) anchor = node;
	}
	for (let i = 0; i < currentChildren.length; i++) if (!used[i]) {
...
function findHydrationEndIndex(nodes, startIdx) {
	for (let j = startIdx + 1; j < nodes.length; j++) if (isHydrationEndComment(nodes[j])) return j;
	return startIdx;
}
...
function disposeRemovedSubFrames(node, context) {
	...
	while (stack.length > 0) {
		...
		for (let child of Array.from(next.childNodes)) stack.push(child);
	}
}
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/frame.js
...
function syncElementAttributes(target, source) {
	for (let attribute of Array.from(target.attributes)) if (!source.hasAttribute(attribute.name)) target.removeAttribute(attribute.name);
	for (let attribute of Array.from(source.attributes)) if (target.getAttribute(attribute.name) !== attribute.value) target.setAttribute(attribute.name, attribute.value);
}
function createFrame(root, init) {
	...
	let frame = createFrameHandle({
		...
		reload: async () => {
			...
			try {
				...
				if (reloadController === controller) frame.dispatchEvent(new Event("reloadComplete"));
			}
		},
	...
	if (frameName) init.namedFrames.set(frameName, frame);
	let context = {
	...
	async function render(content, options) {
		...
		if (container.root instanceof Document && typeof content === "string" && isFullDocumentHtml(content) && typeof content === "string") {
	...
	function createFrameContentRoot() {
		let virtualRoot;
		if (container.root instanceof Document) virtualRoot = createRoot(container.doc.body, {
	...
	function clearFrameContent() {
		for (let node of getContentNodes()) node.parentNode?.removeChild(node);
	}
	...
	function dispose() {
		...
		if (frameName) {
			if (init.namedFrames.get(frameName) === frame) init.namedFrames.delete(frameName);
		}
	}
...
function moveServerStylesToHead(doc, fragment) {
	...
	for (let style of styles) if (style instanceof HTMLStyleElement) target.appendChild(style);
	let heads = Array.from(fragment.querySelectorAll("head"));
	for (let head of heads) if (!head.childNodes.length) head.remove();
}
...
function scheduleHydrationMarker(marker, entry, context, initialHydrationTracker) {
	...
	let hydrateWithComponent = (component) => {
		...
		let vElement = createElement$1(component, entry.props);
	...
	getOrStartModuleLoad(key, entry, marker.id, context).then((component) => {
		if (component) hydrateWithComponent(component);
	}).finally(() => {
		done?.();
	});
}
function getOrStartModuleLoad(key, entry, markerId, context) {
	...
	let loadPromise = (async () => {
		try {
			...
			if (typeof mod !== "function") throw new Error(`Export "${entry.exportName}" from "${entry.moduleUrl}" is not a function`);
...
function createElement$1(component, props) {
	return jsx(component, reviveSerializedValue(props));
}
function reviveSerializedValue(value) {
	if (value === null || value === void 0) return value;
	if (typeof value !== "object") return value;
	if (Array.isArray(value)) return value.map((item) => reviveSerializedValue(item));
	let record = value;
	if (record.$rmxFrame === true) return jsx(Frame, reviveSerializedObject(record.props), reviveSerializedValue(record.key));
	...
	let revived = {};
	for (let key in record) revived[key] = reviveSerializedValue(record[key]);
	return revived;
}
...
function createSubFrames(nodes, context) {
	for (let i = 0; i < nodes.length; i++) {
		...
		if (node.childNodes && node.childNodes.length > 0) createSubFrames(Array.from(node.childNodes), context);
	}
}
function isHydrationMarkerLive(marker, context) {
	...
	if (marker.start.data.trim() !== `rmx:h:${marker.id}`) return false;
	if (marker.end.data.trim() !== "/rmx:h") return false;
	if (!marker.start.parentNode) return false;
...
function removeVirtualRoots(nodes) {
	for (let i = 0; i < nodes.length; i++) {
		...
		if (node.childNodes && node.childNodes.length > 0) removeVirtualRoots(Array.from(node.childNodes));
	}
}
function disposeSubFrames(nodes, context) {
	for (let i = 0; i < nodes.length; i++) {
		...
		if (node.childNodes && node.childNodes.length > 0) disposeSubFrames(Array.from(node.childNodes), context);
	}
}
...
function setupTemplateObserver() {
	...
	let observer = new MutationObserver((mutations) => {
		for (let mutation of mutations) for (let node of mutation.addedNodes) collectAndPublishTemplates(node);
	});
...
function publishFrameTemplate(id, fragment) {
	...
	for (let listener of listeners) listener(fragment.cloneNode(true));
}
function consumeFrameTemplate(id) {
	let queue = bufferedFrameTemplates.get(id);
	if (!queue || queue.length === 0) return null;
	let fragment = queue.shift() ?? null;
	if (queue.length === 0) bufferedFrameTemplates.delete(id);
	return fragment;
}
function subscribeFrameTemplate(id, listener) {
	...
	return () => {
		...
		if (current.size === 0) frameTemplateListeners.delete(id);
	};
}
...
function extractTemplatesFromBuffer(doc, buffer, onTemplate) {
	...
	while (match) {
		...
		if (id) {
			let template = createFragmentFromString(doc, fullMatch).querySelector("template");
			if (template instanceof HTMLTemplateElement && template.id) onTemplate(template.id, template.content);
		}
		cursor = matchEnd;
...
async function renderFrameStream(stream, doc, applyHtml) {
	...
	try {
		while (true) {
			...
			if (parsed.html !== "") {
				html += parsed.html;
				if (!hasBalancedMarkerSummary(collectHtmlMarkerSummary(html))) continue;
		...
		if (html === "" && !appliedOnce) await applyHtml("");
	} finally {
		reader.releaseLock();
	}
}
...
function createElementContainer(root) {
	return {
		doc: root instanceof Document ? root : root.ownerDocument ?? document,
		root,
		get childNodes() {
...
function walkCommentsInNodes(nodes, cb) {
	for (let i = 0; i < nodes.length; i++) {
		...
		if (node.childNodes && node.childNodes.length > 0) walkCommentsInNodes(Array.from(node.childNodes), cb);
	}
}
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/navigation.js
/**
...
function startNavigationListener(signal) {
	...
	navigation.addEventListener("navigate", (event) => {
		...
		let frame = (state.target ? getNamedFrame(state.target) : void 0) ?? topFrame;
		event.intercept({ async handler() {
			if (event.navigationType !== "traverse") navigation.updateCurrentEntry({ state });
			...
			if (state.resetScroll && isNewEntry) window.scrollTo(0, 0);
		} });
	}, { signal });
}
...
function getRuntimeNavigationState(event) {
	if (event.navigationType === "traverse") return getTraverseNavigationState(event);
...
function getTraverseNavigationState(event) {
	let destinationState = event.destination.getState();
	if (isRuntimeNavigation(destinationState)) return destinationState;
	let matchingEntry = window.navigation.entries().find((entry) => entry.key === event.destination.key);
	if (matchingEntry) {
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/run.js
var topFrame;
/**
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/create-element.js
/**
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/event-listeners.js
/**
...
*/
function addEventListeners(target, signal, listeners) {
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/mixins/on-mixin.js
var onMixin = createMixin((handle) => {
	...
	let stableHandler = (event) => {
		...
		reentry = new AbortController();
		currentHandler(event, reentry.signal);
	};
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/mixins/link-mixin.js
var nativeLinkHostTypes = new Set(["a", "area"]);
/**
...
var link = createMixin((handle, hostType) => {
	...
	return (href, options, props) => {
		if (nativeLinkHostTypes.has(hostType)) return renderMixinElement(handle.element, {
			...props ?? {},
			href,
		...
		let nextProps = { ...props };
		if (nextProps.role == null) nextProps.role = "link";
		if (nextProps.disabled === true && nextProps["aria-disabled"] == null) nextProps["aria-disabled"] = "true";
		if (hostType === "button" && nextProps.type == null) nextProps.type = "button";
		if (hostType !== "button" && nextProps.tabIndex == null && nextProps.tabindex == null && nextProps.contentEditable == null && nextProps.contenteditable == null) nextProps.tabIndex = 0;
		return renderMixinElement(handle.element, {
			...
			mix: [
				...
				on("keyup", (event) => {
					if (hostType === "button" && event.key === " ") event.preventDefault();
				})
			]
		});
	};
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/mixins/keys-mixin.js
...
var baseKeysEvents = createMixin((handle) => (props) => renderMixinElement(handle.element, {
	...props ?? {},
	mix: [on("keydown", (event) => {
		let type = keyToEventType[event.key];
		if (!type) return;
		event.preventDefault();
		event.currentTarget.dispatchEvent(new KeyboardEvent(type, { key: event.key }));
	})]
}));
/**
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/mixins/press-mixin.js
...
var basePressEvents = createMixin((handle) => {
	...
	handle.addEventListener("remove", () => {
		...
		if (doc) doc.removeEventListener("pointerup", onDocumentPointerUp);
		target = null;
		doc = null;
		isPointerDown = false;
		isKeyboardDown = false;
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/mixins/ref-mixin.js
/**
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/mixins/css-mixin.js
...
var css = createMixin((handle) => {
	...
	handle.addEventListener("remove", () => {
		...
		resolveStyleTarget(runtime).styleManager?.remove(activeSelector);
		activeSelector = "";
	});
	return (styles, props) => {
		...
		if (styleTarget.styleManager) {
			if (activeSelector && activeSelector !== selector) styleTarget.styleManager.remove(activeSelector);
			if (selector && activeSelector !== selector) styleTarget.styleManager.insert(selector, cssText);
			activeSelector = selector;
		}
		if (!selector) return handle.element;
		return renderMixinElement(handle.element, {
			...props ?? {},
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/mixins/animate-mixins.js
var DEFAULT_ENTER = {
	opacity: 0,
	duration: 150,
	easing: "ease-out"
...
function extractStyleProps(config) {
	...
	for (let key in config) {
		if (key === "duration" || key === "easing" || key === "delay" || key === "composite" || key === "initial") continue;
...
function buildEnterKeyframes(config) {
	return [extractStyleProps(config), {}];
}
...
function collectAnimatedProperties(keyframes) {
	...
	for (let keyframe of keyframes) for (let key in keyframe) {
...
function trackAnimation(node, animation, keyframes) {
	...
	animation.finished.catch(() => {}).finally(() => {
		if (animatingNodes.get(node)?.animation !== animation) return;
...
function waitForAnimationOrAbort(animation, signal) {
	...
	return new Promise((resolve) => {
		...
		animation.finished.catch(() => {}).finally(settle);
	});
}
...
var animateEntranceMixin = createMixin((handle) => {
	let currentConfig = true;
	handle.addEventListener("insert", (event) => {
		let node = event.node;
		let current = animatingNodes.get(node);
		if (current && current.animation.playState === "running") return;
		...
		trackAnimation(node, node.animate(keyframes, options), keyframes);
	});
	return (config) => {
...
* @returns A mixin descriptor for the target element.
*/
function animateEntrance(config = true) {
...
* @returns A mixin descriptor for the target element.
*/
function animateExit(config = true) {
	return animateExitMixin(config);
}
//#endregion
//#region node_modules/@remix-run/component/dist/lib/mixins/animate-layout-mixin.js
...
function calcAxisDelta(delta, source, target, origin = .5) {
	...
	if (isNear(delta.scale, 1, SCALE_PRECISION) || Number.isNaN(delta.scale)) delta.scale = 1;
	if (isNear(delta.translate, 0, TRANSLATE_PRECISION) || Number.isNaN(delta.translate)) delta.translate = 0;
}
function calcBoxDelta(delta, source, target) {
	calcAxisDelta(delta.x, source.x, target.x, .5);
	calcAxisDelta(delta.y, source.y, target.y, .5);
}
...
function buildProjectionTransform(delta) {
	let transform = "";
	if (delta.x.translate || delta.y.translate) transform = `translate3d(${delta.x.translate}px, ${delta.y.translate}px, 0)`;
...
var animateLayoutMixin = createMixin((handle) => {
	...
	let scheduleProgressTracking = (duration, active) => {
		...
		let tick = () => {
			...
			if (animationProgress < 1) requestAnimationFrame(tick);
		};
	...
	handle.addEventListener("beforeUpdate", (event) => {
		if (!resolveLayoutConfig(currentConfig)) return;
...
* @returns A mixin descriptor for the target element.
*/
function animateLayout(config = true) {
	return animateLayoutMixin(config);
}
//#endregion
//#region node_modules/@remix-run/component/dist/lib/spring.js
/**
...
function spring(presetOrOptions, overrides) {
	let { position, settlingTime, easing } = computeSpring(resolveOptions(presetOrOptions, overrides));
...
spring.transition = function transition(property, presetOrOptions, overrides) {
	...
	return (Array.isArray(property) ? property : [property]).map((p) => `${p} ${s}`).join(", ");
};
...
function computeSpring(options) {
	...
	if (zeta < 1) {
		...
		position = (t) => {
			return 1 - Math.exp(-zeta * omega0Ms * t) * ((velocityMs + zeta * omega0Ms) / omegaD * Math.sin(omegaD * t) + Math.cos(omegaD * t));
		};
	} else if (zeta > 1) {
	...
	} else position = (t) => 1 - Math.exp(-omega0Ms * t) * (1 + (velocityMs + omega0Ms) * t);
	let velocitySampleMs = .5;
	function velocityAt(t) {
		if (t < velocitySampleMs) return (position(velocitySampleMs) - position(0)) / velocitySampleMs * 1e3;
...
function generateEasing(position, settlingTime) {
	...
	return `linear(${points.map((p, i) => {
		...
		if (i === 0 || isLast) return value === 1 ? "1" : value.toString();
		return `${value} ${Math.round(p.t / settlingTime * 1e3) / 10}%`;
	}).join(", ")})`;
}
function adaptiveSample(resolve, duration, tolerance = .002, minSegment = 8) {
	let points = [];
	function addPoint(t, value) {
		if (points.length === 0 || points[points.length - 1].t < t) points.push({
			t,
			value
		});
	}
	function subdivide(t0, v0, t1, v1, depth = 0) {
		...
		let vLinear = (v0 + v1) / 2;
		if (Math.abs(vMid - vLinear) > tolerance && t1 - t0 > minSegment) {
...
//#endregion
//#region node_modules/@remix-run/component/dist/lib/tween.js
/**
...
function* tween(options) {
	...
	while (true) {
		...
		let easedProgress = cubicBezier(solveCubicBezierX(x1, x2, linearProgress), y1, y2);
		value = from + (to - from) * easedProgress;
		if (linearProgress >= 1) return to;
	}
}
//#endregion
export { Fragment, Frame, TypedEventTarget, addEventListeners, animateEntrance, animateExit, animateLayout, clientEntry, createElement, createMixin, createRangeRoot, createRoot, createScheduler, css, easings, keysEvents, link, navigate, on, pressEvents, ref, run, spring, tween };

//# sourceMappingURL=remix_component.js.map